...
var channelKeys =[];
//channelKeys.push({channelNumber:71944, name:'temp wifi',key:'222DM9RMFGT6A3GE',
   //fieldList:[{field:1,axis:'T'},{field:2,axis:'T'},{field:3,axis:'T'},{field:4,axis:'T'},{field:5,axis:'T'},{field:6,axis:'T'}]});
//channelKeys.push({channelNumber:161407, name:'RDS',key:'YF9YX6P0CWWQFSJE',
  //    fieldList:[{field:1,axis:'O'},{field:2,axis:'O'},{field:3,axis:'O'},{field:4,axis:'O'},{field:5,axis:'T'},{field:6,axis:'T'},{field:7,axis:'T'},{field:8,axis:'T'}]});
channelKeys.push({channelNumber:283841, name:'RDS',key:'IHEKNAZE3YXDTPK8',
      fieldList:[{field:1,axis:'O'},{field:2,axis:'T'},{field:3,axis:'O'},{field:4,axis:'O'},{field:5,axis:'T'},{field:6,axis:'T'},{field:7,axis:'T'}]});
...
$(document).ready(function()
{
 ...
 function loadThingSpeakChannel(sentChannelIndex,channelNumber,key,sentFieldList) {
   ...
   // get the Channel data with a webservice call
 	$.getJSON('https://www.thingspeak.com/channels/'+channelNumber+'/feed.json?callback=?&amp;offset=0&amp;median=60&amp;results=2500;key='+key, function(data)
   {
	   // if no access
	   if (data == '-1') {
     ...
     for (var fieldIndex=0; fieldIndex<fieldList.length; fieldIndex++)  // iterate through each field
     {
       ...
       for (var h=0; h<data.feeds.length; h++)  // iterate through each feed (data point)
       {
		 ...
		 var q = [] // comparadorde datos
         var fieldStr = "data.feeds["+h+"].field"+fieldList[fieldIndex].field;
		  	var v = eval(fieldStr);
 		  	p[0] = getChartDate(data.feeds[h].created_at);
	 	  	//var fieldStr1 = getChartDate(data.feeds[h].created_at);
			//var fieldStr2 = getChartDate(data.feeds[h].created_at);
			//if(h >= 1 && h < (data.feeds.length-1)){// obtengo el dato anterior y siguiente
			//	fieldStr1 = "data.feeds["+(h-1)+"].field"+fieldList[fieldIndex].field;
			//	fieldStr2 = "data.feeds["+(h+1)+"].field"+fieldList[fieldIndex].field;
			//}else{
			...
			//q[0] = eval(fieldStr1);
			//q[1] = eval(fieldStr2);
			//var promedio = (parseFloat(q[0])+parseFloat(q[1]))/2;
			//window.console && console.log('val promedio:',promedio);
		  	//p[1] = 0;
			//p[1] = parseFloat(v);
		  	//if(parseFloat(v)<(promedio+(promedio)) && parseFloat(v)>(promedio-(promedio)) ){//|| parseFloat(v) < (promedio-(promedio/2))){ // si el valor es 3 veces el promedio no lo muestra
			p[1] = parseFloat(v);
				//window.console && console.log('val ingresado:',parseFloat(v));
			//}
             ...
             if (document.getElementById("Update").checked)
             {
              for (var channelIndex=0; channelIndex<channelKeys.length; channelIndex++)  // iterate through each channel
              {
               (function(channelIndex)
               {
                ...
                $.getJSON('https://www.thingspeak.com/channels/'+channelKeys[channelIndex].channelNumber+'/feed/last.json?callback=?&amp;offset=0&amp;median=60&amp;location=false;key='+channelKeys[channelIndex].key, function(data)
                {
...
function loadChannelHistory(sentChannelIndex,channelNumber,key,sentFieldList,sentNumLoads,maxLoads) {
   ...
   // get the Channel data with a webservice call
 	$.getJSON('https://www.thingspeak.com/channels/'+channelNumber+'/feed.json?callback=?&amp;offset=0&amp;median=60&amp;start=2013-01-20T00:00:00;end='+end+';key='+key, function(data)
   {
	   // if no access
	   if (data == '-1') {
       $('#chart-container').append('This channel is not public.  To embed charts, the channel must be public or a read key must be specified.');
